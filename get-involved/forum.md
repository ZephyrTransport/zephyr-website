---
layout: simple
title: "Zephyr Forum"
subtitle: "Zephyr is piloting the Zephyr Forum, a Discord server for discussions and questions. Join us now!"
section: Get Involved
permalink: /get-involved/forum/
hide-title: true
---

<div class="text-center mb-4">
<img src="{{ '/img/logos/zephyr-forum-logo.png' | relative_url }}" class="img-fluid" alt="Zephyr Forum: An Online Community Built by Travel Modelers. Connect, Learn, Contribute, Advance." style="max-width:min(480px, 100%);"/>
</div>

The **Zephyr Forum** is our online community where transportation and land use professionals ask questions, share ideas, collaborate on projects, and connect through discussion channels, direct messages, voice, and video. *You don't have to be a Zephyr member to join.*

We're launching the Forum as a **pilot**. If the community finds it valuable, it will become Zephyr's primary online discussion space.

<div class="z-note-callout" markdown="1">

**Just want to say hello?**

Joining the Forum doesn't cost anything or commit you to anything further. Hop in, then head to the **#introductions** channel and tell us who you are.

</div>

## Why Discord?

Yes, it's the app gamers made famous. These days it's also where many technical and open-source communities collaborate. Think of it as Slack with better voice channels and a more active community feel.

## What You'll Find

- Ask questions and get technical advice.
- Share ideas, resources, and projects.
- Join the **Zephyr Lounge** for voice and video conversations.
- Connect with transportation professionals throughout the community.

## Forum First Fridays

We're trying out a casual video chat in the **Zephyr Lounge** on the first Friday of every month, starting at **2 PM Eastern** (1 PM Central / 12 PM Mountain / 11 AM Pacific). No agenda, no presentation. Just stop by to say hello, ask a question, or talk shop with other travel modelers.

Zephyr members get a say in the theme: a few days beforehand, we'll run a quick poll in the members channel, and whatever wins becomes that month's loose topic. Everyone's still welcome to just show up, no membership required.

This is a prototype, so the day, time, and format may change as we learn what works.

## Getting Started

The Forum works in any web browser, but we recommend **[installing the Discord desktop or mobile app](https://discord.com/download)** so you'll receive notifications for new messages, mentions, and activity.

<div class="z-note-callout" markdown="1">

**Before you join**

- Access requires administrator approval.
- Please use your real name so others know who they're talking with.
- Zephyr may establish community guidelines and manage access to maintain a welcoming and professional environment.

**Zephyr Lounge Tip**

Entering the Zephyr Lounge automatically connects your audio. There isn't a separate **Join Call** button, so mute your microphone, or be ready to say hello, before entering.

</div>

---
<div class="text-center mt-4 mb-4">
  <a class="btn btn-lg btn-z" href="https://discord.gg/ejR8BK5uP">
    Join the Zephyr Forum on Discord
  </a>
</div>

---